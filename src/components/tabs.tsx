...
export function TabsList(props: React.ComponentPropsWithRef<typeof Unstyled.TabsList>) {
  return (
    <Unstyled.TabsList
      {...props}
      className={cn(
        'flex gap-4 text-fd-secondary-foreground overflow-x-auto px-6 not-prose',
        props.className,
      )}
    />
  );
...
export function TabsTrigger(props: React.ComponentPropsWithRef<typeof Unstyled.TabsTrigger>) {
  return (
    <Unstyled.TabsTrigger
      {...props}
      className={cn(
        'inline-flex items-center gap-2 whitespace-nowrap text-fd-muted-foreground border-b border-transparent py-3 text-base font-medium transition-colors [&_svg]:size-5 hover:text-fd-accent-foreground disabled:pointer-events-none disabled:opacity-50 data-[state=active]:border-fd-primary data-[state=active]:text-fd-primary',
        props.className,
      )}
    />
  );
...
export function Tabs({
  ...
  return (
    <Unstyled.Tabs
      ref={ref}
      className={cn(
        'flex flex-col overflow-hidden rounded-xl border bg-fd-secondary my-8',
        className,
      )}
      value={value}
...
export function TabsContent({
  ...
  return (
    <Unstyled.TabsContent
      ...
      forceMount
      className={cn(
        'p-6 text-base bg-fd-background rounded-xl outline-none prose-no-margin data-[state=inactive]:hidden [&>figure:only-child]:-m-6 [&>figure:only-child]:border-none leading-relaxed',
        className,
      )}
      {...props}
    >
